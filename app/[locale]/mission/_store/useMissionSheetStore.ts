 ...
 *
 * missionId + preview만 보관하는 이유: 시트는 useMissionDetail(missionId)을 구독해 서버
 * 상태를 그대로 쓴다 — Mission 전체 객체를 스토어에 복사해두면 서버 상태의 사본이 Zustand에도
 * 생겨 캐시와 어긋날 수 있다. preview는 상세 응답이 도착하기 전 헤더를 즉시 그리기 위한
 * 최소 정보(제목/난이도/이미지)일 뿐, 진실 원천은 어디까지나 useMissionDetail 쿼리다.
 *
 * open을 missionId의 null 여부와 분리한 이유: 닫힘 exit 모션(300ms) 동안에도 콘텐츠를
 * 그려야 하므로 close()가 missionId를 지우면 안 된다. 대신 openSeq를 열 때마다 올려서
...
/** 상세 응답 도착 전 헤더/요약 카드를 즉시 그리기 위한 최소 정보 */
...
interface MissionSheetState {
  /** 시트 열림 여부 */
  open: boolean;
  /** 시트에 표시할 미션 id. close() 후에도 마지막 값을 유지한다 (exit 모션 중 콘텐츠 보존) */
  ...
  goBackToDetail: () => void;
  /**
   * 시트 닫기. open만 내리고 missionId/preview/step/entry는 일부러 유지한다 —
