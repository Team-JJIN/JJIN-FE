/**
 * 미션 생성 인터셉팅 라우트의 레이아웃 시트. 로딩과 폼 전환 중에도 시트를 유지한다.
...
const MissionCreateDoneContext = createContext<
  ((created?: CreatedMissionResult) => void) | null
>(null);

export function useMissionCreateDone() {
  const onDone = useContext(MissionCreateDoneContext);
  if (!onDone) {
    throw new Error("useMissionCreateDone requires MissionCreateSheet");
  }
  return onDone;
}

export default function MissionCreateSheet({
  children,
}: {
  ...
  return (
    <BottomSheet
      ...
      animated
    >
      <MissionCreateDoneContext.Provider value={handleDone}>
        {children}
      </MissionCreateDoneContext.Provider>
    </BottomSheet>
  );
}
