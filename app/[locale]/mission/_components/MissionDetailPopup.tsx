 ...
 * 그대로 유지된다(아무것도 초기화/커밋되지 않음). '+ 추가' 탭 시에는 즉시 onAddClick을
 * 호출하지 않고 대상 미션을 pendingAddRef에 보관한 채 팝업을 닫기만 하며, exit 모션이
 * 완료된 시점(onExitComplete)에 비로소 상위에서 전달받은 onAddClick을 호출해 딤 배경이
 * 중첩되지 않도록 한다.
...
export default function MissionDetailPopup({
  ...
  const pendingAddRef = useRef<Mission | null>(null);

  // exit 도중 재등장하면 onExitComplete가 발화하지 않아 이전 pendingAdd가 남을 수 있으므로 초기화
  useEffect(() => {
    if (open) pendingAddRef.current = null;
  }, [open]);
  ...
  // '+ 추가' 탭: 즉시 onAddClick하지 않고 exit 완료 후로 미룬다 (딤 중첩 방지)
  const handleAddClick = useCallback(() => {
    if (!mission) return;
    pendingAddRef.current = mission;
    onClose();
  }, [mission, onClose]);
          ...
          >
            <div className="flex justify-end">
              <button
                ...
                onClick={onClose}
                aria-label={t("close")}
                className="text-[20px] leading-none text-neutral-400 transition duration-150 motion-safe:active:scale-90"
              >
                ✕
              </button>
            </div>
